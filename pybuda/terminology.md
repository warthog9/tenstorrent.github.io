...
Hardware unit that copies data from local memory to source registers inside [FPU](#fpu). In the process of copying data, unpacker can also uncompress compressed data, and perform conversion
...
A group of tiles on which a [blocking operation](#blocking-op) is performed.
...
A [Pipe](#pipe) between two buffers, where one copy of data is transferred from source to destination buffer.

## Multicast

A multicast is a [Pipe](#pipe) that has one source buffer but more than one destination. In this case, the NOC will use its multicasting ability to only send one copy of the data along its
...
A gather [Pipe](#pipe) has multiple source buffers and one or more destination buffers. The gather pipe will create connections from source buffers to a single gather buffer, which will then be
...
is performed, and results are written out. HLK is transpiled, using [HLKC](#hlkc), into low-level C++ code that runs on individual RiscV cores. Users who implement new operations would write HLKs
to describe the desired functionality.

## LLK

Low-Level Kernels are implementations of low-level Tensix and NOC API used in [HLKs](#hlk). They use a mix of C++ and assembly to provide highly optimized and performant functionality, such
as data [unpacking](#unpacker), [packing](#packer), running math primitives, and so on. These are typically written by Tenstorrent as they require in-depth knowledge of Tenstorrent
