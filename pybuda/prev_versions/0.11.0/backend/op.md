...
A Buda module is represented as a graph of math operations ([Ops](../terminology.md#op)). Writing out a module is very similar to how it is done in other frameworks, like PyTorch or TensorFlow: operations are
...
split the module into [epochs](../terminology.md#epoch). During an epoch, placement and routing between operations is static, and one or more input tensors are fed through this pipeline of operations until
...
Within an epoch, operations are connected using [pipes](../terminology.md#pipe) and [buffers](../terminology.md#buffer). Each operation defines its own input and output buffers based on its needs, and then creates pipes
...
If this is not compatible with the consuming op, a [TM op](../terminology.md#tm-op) will need to be inserted to bring data into expected form.

Pipes provide for a flexible connectivity between operations, allowing for many kinds of topologies. See [Unicast](../terminology.md#unicast), [Multicast](../terminology.md#multicast), and [Gather](../terminology.md#gather) for more details.
...
The math operation is defined through a high-level kernel ([HLK](../terminology.md#hlk)). A typical HLK looks like simple single-threaded piece of code in which input operands are read, math performed on them, and
output written out.
...
An HLK uses hlk_ API to perform low-level actions in Tensix cores. These involve waiting for [tiles](../terminology.md#tile) to arrive in input buffers, reading them into source operands, running math
primitives, and packing and pushing output data back to output operands. The API implementation is in [low-level kernels](../terminology.md#llk), which are highly optimized pieces of code that abstract away many
of the details of hardware programming.

In addition to HLKs, a Buda op defines [buffers](../terminology.md#buffer) and [pipes](../terminology.md#pipe) needed to move data to and from the operation. The Buda op will create input buffers for each of its operands
based on its requirements. A [Streaming op](../terminology.md#streaming-op) might only need room for two input tiles, while a [Blocking op](../terminology.md#blocking-op) will need to store at least two blocks to be able to perform at 100%. An output
buffer should be sizes to contain at least one output [Block](../terminology.md#block) to prevent deadlocks, but the actual sizing will depend on the performance requirements.

Once the buffers are in places, the Buda op defines pipes to bring data in from incoming operations into its input buffers. Depending on the op requirements, it might need a set of [Unicast](../terminology.md#unicast)
pipes (for example, an elementwise operation), or a [Multicast](../terminology.md#multicast) if several of its cores will be using the same data (most common example of this is a matrix multiplication op).
